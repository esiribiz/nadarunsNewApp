...
class OrderTrackingScreenState extends State<OrderTrackingScreen> {
  ...
  static const MarkerId _deliveryMarkerId = MarkerId('DeliveryBoy');
  ...
  final List<String> _riderFlowStages = [
    ORDER_CREATED,
    ORDER_PENDING,
    ORDER_ASSIGNED,
    ORDER_ACCEPTED,
  ...
  String _statusMessageFor(String status, {String? fallbackMessage}) {
    ...
    switch (status) {
      case ORDER_CREATED:
        return 'Your job has been posted. Waiting for available deliverymen.';
      case ORDER_PENDING:
        return 'Your job is visible to nearby deliverymen.';
      case ORDER_ASSIGNED:
        return 'A deliveryman has been assigned to your job.';
      case ORDER_ACCEPTED:
        return 'Deliveryman accepted and is heading to pickup.';
      case ORDER_ARRIVED:
        return 'Deliveryman arrived at pickup location.';
      case ORDER_PICKED_UP:
        return 'Parcel picked up. Delivery is now in transit.';
      case ORDER_DEPARTED:
        return 'Deliveryman is on the way to drop-off.';
      case ORDER_DELIVERED:
  ...
  int _flowIndex(String status) {
    int index = _riderFlowStages.indexOf(status);
    if (index != -1) return index;

    int historyIndex = -1;
    for (int i = 0; i < _riderFlowStages.length; i++) {
      if (liveOrderHistory.any(
        (history) => history.historyType.validate() == _riderFlowStages[i],
      )) {
        historyIndex = i;
      }
    }
  ...
  Future<void> _refreshMapData(OrderData order) async {
    ...
    int deliveryManId = order.deliveryManId.validate();
    if (deliveryManId > 0 && !_isTerminalStatus(order.status.validate())) {
      try {
        deliveryBoyData = await getUserDetail(deliveryManId);
        double? latitude = _toCoordinate(deliveryBoyData?.latitude);
        double? longitude = _toCoordinate(deliveryBoyData?.longitude);
          ...
          markers.add(
            Marker(
              ...
              infoWindow: InfoWindow(
                ...
                snippet:
                    deliveryBoyData?.updatedAt.validate().isNotEmpty ?? false
  ...
  Widget _buildFlowSummaryCard(OrderData activeOrder) {
    String status = activeOrder.status.validate();
    Color stageColor = _statusColor(status);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          6.height,
          Text(
            _latestStatusTimestamp != null
                ? 'Order #${activeOrder.id.validate()} • Updated ${_formatTime(_latestStatusTimestamp!)}'
                : 'Order #${activeOrder.id.validate()} • Live updates',
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: List.generate(_riderFlowStages.length, (index) {
                String stage = _riderFlowStages[index];
                return Row(
                  children: [
                    _buildFlowStep(
                      stage: stage,
                    ...
                    if (index != _riderFlowStages.length - 1)
                      Container(
                        width: 20,
                        height: 2,
                        margin: const EdgeInsets.only(bottom: 24),
                        color: currentIndex > index
                            ? _statusColor(_riderFlowStages[index + 1])
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                OrderDetailScreen(
                  orderId: activeOrder.id.validate(),
                ).launch(context);
              },
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            left: 12,
            right: 12,
            child: _buildFlowSummaryCard(activeOrder),
          ),
          Positioned(
            left: 12,
            right: 12,
          ...
          if (appStore.isLoading && !_isRefreshing) loaderWidget().center(),
        ],
      ),
    );
  }
