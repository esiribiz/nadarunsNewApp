...
class OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  final List<String> _deliveryTimelineStages = [
    ORDER_CREATED,
    ORDER_ASSIGNED,
    ORDER_ACCEPTED,
    ORDER_PICKED_UP,
  ...
  DateTime _parseHistoryDate(String? dateTime) {
    if (dateTime.isEmptyOrNull) return DateTime.fromMillisecondsSinceEpoch(0);
    return DateTime.tryParse(dateTime.validate())?.toLocal() ?? DateTime.fromMillisecondsSinceEpoch(0);
  }

  String _formatHistoryDate(String? dateTime) {
    if (dateTime.isEmptyOrNull) return '';
    try {
      return printDate(dateTime.validate());
    } catch (_) {
      return dateTime.validate();
    }
  }

  Map<String, OrderHistory> _latestTimelineHistoryByType() {
    Map<String, OrderHistory> timelineHistory = {};
    for (OrderHistory history in orderHistory.validate()) {
      String historyType = history.historyType.validate();
      if (!_deliveryTimelineStages.contains(historyType)) continue;

      OrderHistory? existing = timelineHistory[historyType];
      if (existing == null || _parseHistoryDate(history.createdAt).isAfter(_parseHistoryDate(existing.createdAt))) {
        timelineHistory[historyType] = history;
      }
    }
    return timelineHistory;
  }

  int _currentTimelineIndex(Map<String, OrderHistory> timelineHistory) {
    int statusIndex = _deliveryTimelineStages.indexOf(orderData?.status.validate() ?? '');
    int historyIndex = -1;
    for (int i = 0; i < _deliveryTimelineStages.length; i++) {
      if (timelineHistory.containsKey(_deliveryTimelineStages[i])) {
        historyIndex = i;
      }
    }
    return statusIndex > historyIndex ? statusIndex : historyIndex;
  }

  Widget _buildDeliveryTimelineCard() {
    if (orderData == null || getStringAsync(USER_TYPE) != CLIENT) return SizedBox();

    Map<String, OrderHistory> timelineHistory = _latestTimelineHistoryByType();
    int currentIndex = _currentTimelineIndex(timelineHistory);
    bool isTerminalStatus = orderData!.status == ORDER_CANCELLED || orderData!.status == ORDER_FAIL;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(language.orderHistory, style: boldTextStyle(size: 16)),
          12.height,
          ...List.generate(_deliveryTimelineStages.length, (index) {
            String stage = _deliveryTimelineStages[index];
            OrderHistory? stageHistory = timelineHistory[stage];
            bool isCompleted = currentIndex >= index;
            bool isCurrent = !isTerminalStatus && currentIndex == index && orderData!.status != ORDER_DELIVERED;
            bool isLast = index == _deliveryTimelineStages.length - 1;
            Color activeColor = ColorUtils.colorPrimary;
            Color inactiveColor = Colors.grey.withValues(alpha: 0.5);
            String stageDateText = _formatHistoryDate(stageHistory?.createdAt);
                  ...
                  children: [
                    Text(
                      ...
                      style: primaryTextStyle(
                        size: 13,
                        color: isCompleted ? activeColor : null,
                      ),
                    ),
                    if (stageDateText.isNotEmpty)
                      Text(stageDateText, style: secondaryTextStyle(size: 11))
                    else if (isCurrent)
                      Text('In progress', style: secondaryTextStyle(size: 11)),
                  ],
                ).expand(),
              ],
            );
          }),
          if (orderData!.status == ORDER_CANCELLED || orderData!.status == ORDER_FAIL) ...[
            8.height,
            Text(
              'This order was ${orderStatus(orderData!.status.validate()).toLowerCase()}.',
              ...
              : !appStore.isLoading
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          Text(language.noDataFound, style: secondaryTextStyle()),
                          12.height,
                          AppButton(
                            elevation: 0,
                            color: ColorUtils.colorPrimary,
                            child: Text('Retry', style: primaryTextStyle(color: white)),
                            onTap: () {
                              orderDetailApiCall();
                            },
                          ),
