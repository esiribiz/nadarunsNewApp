...
class LoginScreenState extends State<LoginScreen> {
  ...
  String readableAuthError(dynamic error) {
    String message = error.toString();
    if (message.contains('invalid_username')) return language.emailInvalid;
    if (message.contains('auth.failed')) return language.credentialNotMatch;
    if (message.contains('Timeout')) return language.errorSomethingWentWrong;
    if (message.contains('message:')) {
      return message.replaceAll(RegExp(r'[\{\}]'), '').replaceAll('message:', '').trim();
    }
    return message;
  }
  ...
  Future<void> loginApiCall() async {
    if (formKey.currentState!.validate()) {
      ...
      try {
        ...
        final v = await logInApi(req);

        // Non-blocking Firebase sign-in (chat sync), should not block app login.
        authService.signInWithEmailPassword(context, email: emailController.text, password: passController.text).catchError((e) {
          log("Firebase sign-in warning: $e");
        });
